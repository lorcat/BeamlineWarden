...
class Logger(object):
    ...
    def __init__(self, def_file=None, debug_level=None):
        ...
        logging.basicConfig(level=self.DEFAULTLEVEL,
                            format=config.LOGGING_MAIN_FORMAT,
                            datefmt=config.LOGGING_MAIN_DATE,
                            filemode='w')
    ...
    def addFileHandler(self, filename):
        ...
        # add formatter
        fmtr = logging.Formatter(config.LOGGING_FILE_FORMAT,
                                 config.LOGGING_FILE_DATE)
...
class MutexLock(Tester):
    DEFAULT_FILE = "lock"
    DEFAULT_DIR = config.LOCKING_DIR
    DEF_APP_DIR = config.LOCKING_LOCAL_DIR
    ...
    def __init__(self, def_file=None, debug_level=None):
        ...
        if not os.path.exists(self.DEFAULT_DIR) or not os.path.isdir(self.DEFAULT_DIR):
            self.lock_name = os.path.join(config.FILE_ROOT, *temp)
            self.lock_dir = os.path.join(config.FILE_ROOT, self.DEF_APP_DIR)
        else:
